...
local SwitchController = class("controls.SwitchController", PlayerController)

local SWITCH_COOLDOWN = 0.1
local SWITCH_MAX_SPEED = 10

function SwitchController:enter(binding, ship_left, ship_right, chain)
	PlayerController.enter(self, binding, ship_left, ship_right, chain)

	self.switch_cooldown = 0
end

function SwitchController:update(dt, rt)
	...
	if self.ship_left.x > self.ship_right.x and self.switch_cooldown <= 0 then
